...
from .acados_ocp_pp_baseline import acados_ocp_pp_baseline
...
class MPC_baseline:
    """Baseline Model Predictive Controller for autonomous vehicle racing."""
    ...
    def __init__(self, vehicle_model, track, config, control_init=None):
        ...
        acados_ocp = acados_ocp_pp_baseline(vehicle_model, track, config)
        self.__solver = acados_solver(acados_ocp, config, config.acados.export_dir_baseline)
        self.__solver_for_decision = acados_solver(acados_ocp, config, config.acados.export_dir_baseline_dec)
        ...
        self.SIM_STEP = 0
        self.export_dir = 'env/mpc/' + config.acados.export_dir_baseline
        self.export_dir_dec = 'env/mpc/' + config.acados.export_dir_baseline_dec
    ...
    def _compute_following_trajectory(self, theta0, ey0, obstacle_trajectories, epsilon=0.0):
        """Compute following trajectory via interpolation."""
    ...
    def _build_solver_params(self, k, theta0, driving_mode, driving_mode_other,
        ...
        """Build parameter vector for solver at stage k."""
    ...
    def _convert_to_cartesian(self, trajectory, state):
        """Convert trajectory from curvilinear to Cartesian coordinates."""
        ...
        # Add longitudinal error correction
    ...
    def _solve_core(self, state, driving_mode, driving_mode_other,
                    ...
                    solver, epsilon=0.0):
        """
        Core MPC solve logic shared between solve() and solve_for_decision().

        Returns:
            tuple: (status, trajectory, theta_next, ey0, kapparefhorizon)
        """
        ...
        # Initialize states if first step
        ...
        # Precompute following trajectory if needed
        following_traj = None
    ...
    def solve(self, state, state_tv, theta_TV, driving_mode, driving_mode_other,
        ...
        """Solve MPC problem and update internal state."""
        # Initialize defaults
        ...
        # Also initialize decision solver for consistency
    ...
    def solve_for_decision(self, state, state_tv, theta_TV, driving_mode, driving_mode_other,
                           obstacle_trajectories=None, sampled_traj=None, sampled_traj_mean=None,
                           sampled_traj_log_var_x=None, sampled_traj_log_var_y=None):
        """Solve MPC for decision making without updating internal trajectory."""
        # Initialize defaults
        sampled_traj_mean, sampled_traj_log_var_x, sampled_traj_log_var_y = \
            self._init_sampled_traj_defaults(
                sampled_traj_mean, sampled_traj_log_var_x, sampled_traj_log_var_y
    ...
    def try_load_iterate(self, solver, path):
        """Try to load warm-start file, reset if not found or failed."""
        if not path.is_file():
